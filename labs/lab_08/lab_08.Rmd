...
**A2. The top ten counties with the lowest democratic ballots percentage return are Carroll, Anne Arundel, Cecil, Howard, Charles, Baltimore County, Frederick, Prince George's, Montgomery, and Baltimore city. For the most part, republican return ballots percentage is lower than Democratics and is only higher in Charles county.**

```{r}
md_absentee_ballots <- md_absentee_ballots |>
mutate(dem_pct_return = (dem_received/ dem_sent) * 100) |>
  mutate(rep_pct_return = (rep_received/ rep_sent) * 100) 
 top10_ballots_wit_low_pct <- md_absentee_ballots |> arrange((dem_pct_return)) |>
 select(county_name, dem_pct_return, rep_pct_return) |> 
 head(10)
 
 top10_ballots_wit_low_pct |> 
 ggplot() +
...
**A3: According to the line graph, Democrats had the most absentee ballots received, while Republicans had the second most while Others had the least.**
...
**A4: Looks like bigger counties sent more and received more ballots. For example, Montgomery county is overperforming and is at the top of the relationship between ballots sent and ballots received. While counties like Allegany and Somerset**
