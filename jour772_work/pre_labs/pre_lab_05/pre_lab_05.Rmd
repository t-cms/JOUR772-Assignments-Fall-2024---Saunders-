...
fentanyl_deaths
#col_names =FALSE- is saying don't count or assume the first row is the header name, so telling R to remove the headers 
```
...
fentanyl_deaths
#renaming the column headers to actual column names using the rename() function 
#new name becomes before the old name in the rename () function 
```
...
#after renaming the column headers, we are now adding a new column with the same now (so basically replacing that column) to remove the jursidcition entry columns periods from the table-- str_replace_all- in the column jursidiction whenver you find a period (\\.-look for period) with nothing (''- represents nothing)
#str_squish- removing all the white spaces from the left to the right 
```
