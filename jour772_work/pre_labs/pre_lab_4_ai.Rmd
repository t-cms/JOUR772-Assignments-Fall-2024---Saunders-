...
The first thing to know about the large language models that have attracted so much attention, money and coverage is this: they are not fact machines. But they are - mostly - very useful for people who write code and for those trying to work through complex problems. That's you. At its core, what a large language model does is predict the next word in a phrase or sentence. They are probabilistic prediction machines based on a huge set of training data. This pre-lab goes through some tasks and examples using LLMs.
...
evans_response <- axolotr::ask(
  prompt = paste("Given the following text, extract information into a CSV file with the following structure, including headers, with no yapping: celebrant1,celebrant2,location_city, location_state,officiant,celebrant1_age,celebrant2_age,celebrant1_parent1,celebrant1_parent2,celebrant2_parent1,celebrant2_parent2", text),
  model = "llama-3.1-8b-instant"
)
...
Now we can turn that object into an R dataframe, using Tidyverse's read_csv()

```{r}
evans_response_df <- read_csv(evans_response)
```

Pretty good, right? Now let's try again, but this time without telling Groq the column names. Let's have it figure it out by itself.\

```{r}
evans_response_no_column_names <- axolotr::ask(
  prompt = paste("Given the following text, extract relevant information into a CSV with a structure that makes sense to recognize all entities and their features. Each entity should be a row, and their features should be columns. Give me just the CSV, with no yapping and no title", text),
  model = "llama-3.1-8b-instant"
)
evans_response_no_column_names <- read_csv(evans_response_no_column_names)
```
...
## TO DO: Create code to ask Groq anything related to this class. Look at its response and comment on whether it is appropriate and correct, and your thoughts about it.

```{r}
filter_inquiry <- axolotr::ask(
  prompt = "Explain how to filter in R",
  model = "llama-3.1-8b-instant"
)
