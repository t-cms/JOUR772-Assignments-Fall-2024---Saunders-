...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? **Answer**
...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: The two N/A's, the L for loan, and the contract as a category.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer: It lowercases the column names and replace spaces with underlines to make our coding process more efficient.**
...
```{r}
cleaned_md_grants_loans |>
  group_by(zip_code, source,grantee) |>
  summarize(total_amount = sum(amount, na.rm = TRUE)) |>
  arrange(desc(total_amount)) 
  
#couldn't figure out function to pull Maryland Tourism Board to show which zip code has gotten the most, but I was able to arrange the total amounts in descending order for the dataset. 
